    ...
    def __init__(self):
        ...
        self.data = {
            'ru': {
                ...
                'Версия: 1.4 (program hub compiler)': 'Версия: 1.4 (program hub compiler)',
                'Тимлид: Васильев Антон Романович': 'Тимлид: Васильев Антон Романович',
                'Команда разработчиков: Дипсик Дипсикович': 'Команда разработчиков: Дипсик Дипсикович',
            ...
            'en': {
                ...
                'Версия: 1.4 (program hub compiler)': 'Version: 1.4 (program hub compiler)',
                'Тимлид: Васильев Антон Романович': 'Timlead: Anton Romanovich Vasiliev',
                'Команда разработчиков: Дипсик Дипсикович': 'Development team: Dipsik Dipsikovich',
...
class CodeEditor(QPlainTextEdit):
    ...
    def lineNumberAreaPaintEvent(self, event):
        ...
        painter.fillRect(event.rect(), QColor(40, 44, 52))  # Изменен цвет фона нумерации
        ...
        while block.isValid() and top <= event.rect().bottom():
            if block.isVisible() and bottom >= event.rect().top():
                ...
                painter.setPen(QColor(220, 220, 220))  # Более светлый цвет для номеров строк
    ...
    def highlight_current_line(self):
        ...
        if not self.isReadOnly():
            ...
            line_color = QColor(50, 55, 65)  # Изменен цвет выделения текущей строки
...
class SyntaxHighlighter(QSyntaxHighlighter):
    def __init__(self, parent=None):
        ...
        keyword_format.setForeground(QColor('#FFA500'))  # Изменен синий на оранжевый для лучшей видимости
        ...
        string_format.setForeground(QColor('#98FB98'))  # Изменен зеленый на более светлый
        pattern = QRegularExpression(r'"[^"\\]*(\\.[^"\\]*)*"|\'[^\'\\]*(\\.[^\'\\]*)*\'')
        rule = (pattern, string_format)
        self.highlighting_rules.append(rule)

        comment_format = QTextCharFormat()
        comment_format.setForeground(QColor('#BC8F8F'))  # Изменен серый на коричневатый
        pattern = QRegularExpression(r'#.*')
...
class EditorTab(QWidget):
    def __init__(self):
        ...
        # Добавлен стиль для редактора с рамкой
...
class TextEditor(QMainWindow):
    ...
    def initUI(self):
        # Настройка стиля для всего окна
        self.setStyleSheet("""
            QMainWindow {
                background-color: #2b2b2b;
            }
        ...
        # Обновлен стиль для текста результатов
    ...
    def update_toolbar(self):
        ...
        def create_icon(name):
            if name == 'system-run':
                ...
                painter.setBrush(QColor(255, 165, 0))  # Изменен цвет на оранжевый
...
"""
    ...
    def show_about(self):
        QMessageBox.about(
            ...
            self.tr("Версия: 1.4 (ph compiler)") + "\n\n" +
            self.tr("Тимлид: Васильев Антон Романович") + "\n" +
            self.tr("Команда разработчиков: Дипсик Дипсикович") + "\n\n" +
...
def main():
    ...
    palette.setColor(QPalette.ColorRole.Highlight, QColor(255, 165, 0))  # Оранжевый highlight
